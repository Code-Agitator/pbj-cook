...
def test_ingredient_list_accessible_to_anyone(client, family):
    ...
    response = client.get(f"/api/meals/{meal_id}/ingredient-list", headers=family["admin_headers"])
    assert response.status_code == 200
...
def test_ingredient_list_deduplicates_dishes_and_excludes_skipped(client, family):
    ...
    # 非登录用户也可以访问食材清单
    response = client.get(f"/api/meals/{meal_id}/ingredient-list")
    assert response.status_code == 200
    items = {item["name"]: item for item in response.json()["items"]}
    assert items == {
        "鸡蛋": {"name": "鸡蛋", "unit": "个", "total": 2, "fragments": []},
